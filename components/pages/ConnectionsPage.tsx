      ...
      <header className={`p-6 sticky top-0 z-20 shadow-xl transition-colors duration-500 ${isGhostMode ? 'bg-[var(--app-bg-ghost)]' : 'bg-[var(--app-primary)] text-white'}`}>
        ...
        <div className="flex p-1 bg-white/5 rounded-2xl border border-white/5 mb-6">
          <button 
            ...
            className={`flex-1 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'followers' ? (isGhostMode ? 'bg-[var(--app-accent)] text-[var(--app-primary)]' : 'bg-[var(--app-accent)] text-[var(--app-primary)]') + ' font-bold shadow-lg' : 'text-white/40'}`}
          >
            Followers
          </button>
          <button 
            onClick={() => setActiveTab('following')}
            className={`flex-1 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'following' ? (isGhostMode ? 'bg-[var(--app-accent)] text-[var(--app-primary)]' : 'bg-[var(--app-accent)] text-[var(--app-primary)]') + ' font-bold shadow-lg' : 'text-white/40'}`}
          >
            Following
          </button>
        </div>
        ...
        <div className="relative">
          ...
          <input 
            ...
            className="w-full bg-white/10 border border-white/10 py-3 pl-12 pr-4 rounded-xl text-sm focus:outline-none focus:bg-white/20 transition-all placeholder:text-primary/20 dark:text-white/20"
          />
        </div>
      </header>
        ...
        {filteredList.map((user) => (
          ...
          >
            ...
            <div className="flex items-center gap-2">
              ...
              <button className="p-2 rounded-xl text-primary/20 dark:text-white/20">
