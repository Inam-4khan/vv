...
export const ConnectionsPage: React.FC<ConnectionsPageProps> = React.memo(({ onBack, onViewProfile, isGhostMode }) => {
  ...
  const followers = MOCK_USERS.slice(1, 5);
  const following = MOCK_USERS.slice(0, 4);
      ...
      <header className={`p-6 sticky top-0 z-20 shadow-xl transition-colors duration-500 ${isGhostMode ? 'bg-[var(--app-bg-ghost)]' : 'bg-[var(--app-primary)] text-white'}`}>
        ...
        <div className="flex p-1 bg-white/5 rounded-2xl border border-white/5 mb-6">
          <button 
            ...
            className={`flex-1 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'followers' ? (isGhostMode ? 'bg-[var(--app-accent)] text-[#062B34] font-black' : 'bg-[var(--app-accent)] text-[#062B34] font-black') + ' font-bold shadow-lg' : 'text-slate-500 dark:text-slate-400'}`}
          >
            Followers
          </button>
          <button 
            onClick={() => setActiveTab('following')}
            className={`flex-1 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'following' ? (isGhostMode ? 'bg-[var(--app-accent)] text-[#062B34] font-black' : 'bg-[var(--app-accent)] text-[#062B34] font-black') + ' font-bold shadow-lg' : 'text-slate-500 dark:text-slate-400'}`}
          >
            Following
          </button>
        </div>
        ...
        <div className="relative">
          ...
          <input 
            ...
            className="w-full bg-white/10 border border-white/10 py-3 pl-12 pr-4 rounded-xl text-sm focus:outline-none focus:bg-white/20 transition-all placeholder:text-slate-500 dark:text-slate-400 "
          />
        </div>
      </header>
      ...
      <div className="p-4 space-y-3 flex-1 overflow-y-auto no-scrollbar">
        {filteredList.map((user) => (
          <div 
            ...
            className={`p-4 rounded-[2rem] border flex items-center gap-4 transition-all active:scale-[0.98] cursor-pointer ${isGhostMode ? 'bg-[var(--app-bg-ghost)] border-[color-mix(in_srgb,var(--app-accent)_10%,transparent)]' : 'bg-[var(--app-bg-surface)] border-black/5 dark:border-white/10 shadow-sm text-slate-900 dark:text-[#F1FAEE] '}`}
          >
            <div className="relative">
              <OptimizedImg src={user.avatar} loading="lazy" className={`w-14 h-14 rounded-2xl border-2 ${isGhostMode ? 'border-[color-mix(in_srgb,var(--app-accent)_20%,transparent)] grayscale opacity-70' : 'border-[var(--app-primary)]'}`} alt={user.displayName} />
            ...
            <div className="flex-1 min-w-0">
              <h4 className="font-bold truncate text-slate-900 dark:text-[#F1FAEE] ">@{user.username}</h4>
              <p className="text-[10px] font-bold uppercase tracking-widest opacity-40 text-slate-900 dark:text-[#F1FAEE] ">{user.displayName}</p>
            </div>
            <div className="flex items-center gap-2">
              ...
              <button className="p-2 rounded-xl text-slate-500 dark:text-slate-400 ">
