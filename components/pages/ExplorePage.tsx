      ...
      <header className={`p-6 text-primary dark:text-white sticky top-0 z-20 shadow-xl transition-colors duration-500 ${isGhostMode ? 'bg-[var(--app-bg-ghost)]' : 'bg-[var(--app-primary)]'}`}>
        ...
        <div className="relative">
          ...
          <input 
            ...
            className="w-full bg-white/10 border border-white/10 py-3 pl-12 pr-12 rounded-2xl text-sm focus:outline-none focus:bg-white/20 transition-all placeholder:text-primary dark:text-white/20"
          />
          ...
          {(['all', 'people', 'spots'] as const).map((tab) => (
            <button
              ...
              className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-wider transition-all ${
                activeTab === tab
                  ? 'bg-[var(--app-accent)] text-primary dark:text-white'
                  : 'bg-white/10 text-primary/40 dark:text-white/40 hover:text-primary dark:text-white'
              }`}
            >
              {tab}
            </button>
      ...
      <div className="p-6 space-y-8 flex-1 overflow-y-auto no-scrollbar">
        ...
        <section className="animate-fade-in">
          ...
          <div className="grid grid-cols-3 gap-3">
            ...
            <button className={`flex flex-col items-center gap-4 p-5 rounded-[2.5rem] border transition-all active:scale-95 shadow-sm ${isGhostMode ? 'bg-[var(--app-bg-ghost)] border-[color-mix(in_srgb,var(--app-accent)_10%,transparent)] text-primary dark:text-white hover:border-[color-mix(in_srgb,var(--app-accent)_30%,transparent)]' : 'bg-[var(--app-bg-surface)] border-black/5 dark:border-white/10 text-primary dark:text-white  hover:border-[color-mix(in_srgb,var(--app-accent)_20%,transparent)]'}`}>
              <div className={`w-14 h-14 rounded-2xl flex items-center justify-center shadow-lg ${isGhostMode ? 'bg-[color-mix(in_srgb,var(--app-accent)_10%,transparent)] text-[var(--app-accent-light)]' : 'bg-primary/10 text-primary dark:text-white'}`}>
                <Video size={28} />
              </div>
           ...
           {['Trending', 'New Personas', 'Hot Spots', 'Audio Hubs'].map((tab, idx) => (
             <button 
              ...
              className={`px-5 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest whitespace-nowrap transition-all border ${
                idx === 0 
                ? (isGhostMode ? 'bg-[var(--app-accent)] border-[var(--app-accent)] text-primary dark:text-white' : 'bg-[var(--app-accent)] border-[var(--app-accent)] text-primary dark:text-white') + ' font-black shadow-lg' 
        ...
        <section className="animate-fade-in">
           ...
           <div className={`relative h-48 rounded-[3rem] overflow-hidden shadow-2xl border-4 group transition-all duration-500 ${isGhostMode ? 'border-[color-mix(in_srgb,var(--app-accent)_20%,transparent)] shadow-[color-mix(in_srgb,var(--app-accent)_10%,transparent)]' : 'border-white/10 shadow-black/10'}`}>
              ...
              <div className="absolute bottom-6 left-6 right-6 flex justify-between items-end">
                 <div>
                    <h4 className="text-primary dark:text-white font-bold text-xl leading-tight mb-1">Camden Town Hub</h4>
                    <p className="text-primary/40 dark:text-white/40 text-[10px] uppercase font-black tracking-[0.2em] flex items-center gap-2">
                       <Plus size={12} /> 1.2km away • 42 personas
                    </p>
                 </div>
                 <button className={`p-4 text-primary dark:text-white rounded-[1.5rem] shadow-2xl active:scale-90 transition-all ${isGhostMode ? 'bg-[var(--app-accent)] text-primary dark:text-white' : 'bg-[var(--app-accent)] text-primary dark:text-white'} group-hover:rotate-6 font-bold`}>
                    <Compass size={24} />
                 </button>
              </div>
           </div>
                    ...
                    <div className={`w-28 h-28 rounded-[2.5rem] p-0.5 shadow-xl transition-all group-hover:scale-105 group-hover:-rotate-3 overflow-hidden border relative ${isGhostMode ? 'bg-[var(--app-bg-ghost)] border-[color-mix(in_srgb,var(--app-accent)_20%,transparent)]' : 'bg-[var(--app-bg-surface)] border-white/10'}`}>
                       ...
                       <div className={`absolute inset-x-0 bottom-0 p-1.5 backdrop-blur-md flex flex-wrap gap-1 justify-center border-t ${isGhostMode ? 'bg-[color-mix(in_srgb,var(--app-bg-ghost)_70%,transparent)] border-[color-mix(in_srgb,var(--app-accent)_20%,transparent)]' : 'bg-[color-mix(in_srgb,var(--app-bg-surface)_70%,transparent)] border-white/10'}`}>
                         {user.interests?.slice(0, 2).map(int => (
                             <span key={int} className={`text-[7px] font-black px-2 py-0.5 rounded-full uppercase tracking-tighter shadow-sm ${isGhostMode ? 'bg-[var(--app-accent)] text-primary dark:text-white' : 'bg-[var(--app-accent)] text-primary dark:text-white'}`}>{int}</span>
                         ))}
                       </div>
                    </div>
                  </div>
