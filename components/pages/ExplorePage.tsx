...
export const ExplorePage: React.FC<ExplorePageProps> = React.memo(({ onBack, isGhostMode }) => {
  ...
  return (
    <div className={`min-h-full flex flex-col transition-colors duration-500 ${isGhostMode ? 'bg-[var(--app-bg-ghost)] text-[#F1FAEE]' : 'bg-[var(--app-bg)] text-slate-900 dark:text-[#F1FAEE]'}`}>
        ...
        <div className="max-w-4xl mx-auto w-full">
          ...
          <div className="relative">
            ...
            <input 
              ...
              className="w-full bg-white/10 border border-white/10 py-3 pl-12 pr-12 rounded-2xl text-sm focus:outline-none focus:bg-white/20 transition-all placeholder:text-slate-500 dark:text-slate-400"
            />
            ...
            {(['all', 'people', 'spots'] as const).map((tab) => (
              <button
                ...
                className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-wider transition-all ${
                  activeTab === tab
                    ? 'bg-[var(--app-accent)] text-[#062B34] font-black'
                    : 'bg-white/10 text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:text-[#F1FAEE]'
                }`}
              >
                {tab}
              </button>
      ...
      <div className="p-6 space-y-8 flex-1 overflow-y-auto no-scrollbar max-w-4xl mx-auto w-full">
        {/* Media Exploration Tiles */}
        <section className="animate-fade-in">
          <h3 className="text-[10px] font-black uppercase tracking-[0.3em] mb-4 px-1 text-slate-500 dark:text-slate-400 ">Media Discovery</h3>
          <div className="grid grid-cols-3 gap-3">
            <button className={`flex flex-col items-center gap-4 p-5 rounded-[2.5rem] border transition-all active:scale-95 shadow-sm ${isGhostMode ? 'bg-[var(--app-bg-ghost)] border-[color-mix(in_srgb,var(--app-accent)_10%,transparent)] text-slate-900 dark:text-[#F1FAEE] hover:border-[color-mix(in_srgb,var(--app-accent)_30%,transparent)]' : 'bg-[var(--app-bg-surface)] border-black/5 dark:border-white/10 text-slate-900 dark:text-[#F1FAEE]  hover:border-[color-mix(in_srgb,var(--app-accent)_20%,transparent)]'}`}>
            ...
            <button className={`flex flex-col items-center gap-4 p-5 rounded-[2.5rem] border transition-all active:scale-95 shadow-sm ${isGhostMode ? 'bg-[var(--app-bg-ghost)] border-[color-mix(in_srgb,var(--app-accent)_10%,transparent)] text-slate-900 dark:text-[#F1FAEE] hover:border-[color-mix(in_srgb,var(--app-accent)_30%,transparent)]' : 'bg-[var(--app-bg-surface)] border-black/5 dark:border-white/10 text-slate-900 dark:text-[#F1FAEE]  hover:border-[color-mix(in_srgb,var(--app-accent)_20%,transparent)]'}`}>
              <div className={`w-14 h-14 rounded-2xl flex items-center justify-center shadow-lg ${isGhostMode ? 'bg-[color-mix(in_srgb,var(--app-accent)_10%,transparent)] text-[var(--app-accent-light)]' : 'bg-primary/10 text-slate-900 dark:text-[#F1FAEE]'}`}>
                <Video size={28} />
              </div>
           ...
           {['Trending', 'New Personas', 'Hot Spots', 'Audio Hubs'].map((tab, idx) => (
             <button 
              ...
              className={`px-5 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest whitespace-nowrap transition-all border ${
                idx === 0 
                ? (isGhostMode ? 'bg-[var(--app-accent)] border-[var(--app-accent)] text-slate-900 dark:text-[#F1FAEE]' : 'bg-[var(--app-accent)] border-[var(--app-accent)] text-slate-900 dark:text-[#F1FAEE]') + ' font-black shadow-lg' 
                : (isGhostMode ? 'bg-white/5 text-slate-500 dark:text-slate-400  border-transparent hover:border-white/10' : 'bg-white/5 text-slate-500 dark:text-slate-400  border-transparent hover:border-white/10')
              }`}
             >
               {tab}
             </button>
        ...
        <section className="animate-fade-in">
           <div className="flex justify-between items-center mb-4 px-1">
              <h3 className={`text-[10px] font-black uppercase tracking-[0.3em] flex items-center gap-2 ${isGhostMode ? 'text-slate-500 dark:text-slate-400 ' : 'text-slate-500 dark:text-slate-400 '}`}>
           ...
           <div className={`relative h-48 rounded-[3rem] overflow-hidden shadow-2xl border-4 group transition-all duration-500 ${isGhostMode ? 'border-[color-mix(in_srgb,var(--app-accent)_20%,transparent)] shadow-[color-mix(in_srgb,var(--app-accent)_10%,transparent)]' : 'border-white/10 shadow-black/10'}`}>
              <OptimizedImg src="https://images.unsplash.com/photo-1526778548025-fa2f459cd5c1?auto=format&fit=crop&w=800&q=80" loading="lazy" className={`w-full h-full object-cover transition-transform duration-1000 group-hover:scale-110 ${isGhostMode ? 'grayscale brightness-50' : ''}`} alt="Map Preview" />
              ...
              <div className="absolute bottom-6 left-6 right-6 flex justify-between items-end">
                 <div>
                    ...
                    <p className="text-white/80 text-[10px] uppercase font-black tracking-[0.2em] flex items-center gap-2">
                       <Plus size={12} /> 1.2km away • 42 personas
                    </p>
                 </div>
                 <button className={`p-4 rounded-[1.5rem] shadow-2xl active:scale-90 transition-all ${isGhostMode ? 'bg-[var(--app-accent)] text-[#062B34] font-black' : 'bg-[var(--app-accent)] text-[#062B34] font-black'} group-hover:rotate-6 font-bold`}>
                    <Compass size={24} />
                 </button>
              </div>
           </div>
        ...
        <section className="animate-fade-in delay-100">
           <div className="flex justify-between items-center mb-5 px-1">
              ...
              <button className="text-[10px] font-black uppercase tracking-widest text-slate-500 dark:text-slate-400  hover:text-[var(--app-accent)] transition-colors">Global View</button>
           </div>
                ...
                <div key={user.id} className="flex flex-col items-center gap-3 shrink-0 group cursor-pointer transition-all active:scale-95">
                  <div className="relative">
                    <div className={`w-28 h-28 rounded-[2.5rem] p-0.5 shadow-xl transition-all group-hover:scale-105 group-hover:-rotate-3 overflow-hidden border relative ${isGhostMode ? 'bg-[var(--app-bg-ghost)] border-[color-mix(in_srgb,var(--app-accent)_20%,transparent)]' : 'bg-[var(--app-bg-surface)] border-white/10'}`}>
                       <OptimizedImg src={user.avatar} loading="lazy" className={`w-full h-full object-cover rounded-[2.3rem] ${isGhostMode ? 'grayscale opacity-70' : ''}`} alt={user.displayName} />
                       ...
                       <div className={`absolute inset-x-0 bottom-0 p-1.5 backdrop-blur-md flex flex-wrap gap-1 justify-center border-t ${isGhostMode ? 'bg-[color-mix(in_srgb,var(--app-bg-ghost)_70%,transparent)] border-[color-mix(in_srgb,var(--app-accent)_20%,transparent)]' : 'bg-[color-mix(in_srgb,var(--app-bg-surface)_70%,transparent)] border-white/10'}`}>
                         {user.interests?.slice(0, 2).map(int => (
                             <span key={int} className={`text-[7px] font-black px-2 py-0.5 rounded-full uppercase tracking-tighter shadow-sm ${isGhostMode ? 'bg-[var(--app-accent)] text-[#062B34] font-black' : 'bg-[var(--app-accent)] text-[#062B34] font-black'}`}>{int}</span>
                         ))}
                       </div>
                    </div>
                  </div>
                  <div className="text-center w-28">
                    <p className="text-xs font-bold truncate mb-0.5 text-slate-900 dark:text-[#F1FAEE] ">@{user.username}</p>
