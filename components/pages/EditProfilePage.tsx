...
export const CARD_SKINS = [
  { id: 'default', name: 'Default', bg: 'bg-white', text: 'text-slate-900 dark:text-[#F1FAEE]', border: 'border-gray-200' },
  { id: 'holographic', name: 'Holographic', bg: 'bg-[linear-gradient(135deg,var(--app-accent-light)_0%,var(--app-accent)_40%,#20878E_100%)]', text: 'text-slate-900 dark:text-[#F1FAEE]', border: 'border-[rgba(128,255,236,0.60)]' },
...
export const EditProfilePage: React.FC<EditProfilePageProps> = React.memo(({ user, onBack, onSave, isGhostMode }) => {
  ...
  const [banner, setBanner] = useState("https://images.unsplash.com/photo-1618005182384-a83a8bd57fbe?auto=format&fit=crop&w=800&q=80"); 
  ...
  const handleBannerClick = () => {
    ...
    setTimeout(() => {
      setBanner(`https://images.unsplash.com/photo-1550745165-9bc0b252726f?auto=format&fit=crop&w=800&q=80`);
  ...
  return (
    <div className={`min-h-full flex flex-col transition-colors duration-500 ${isGhostMode ? 'bg-[var(--app-bg-ghost)] text-[#F1FAEE]' : 'bg-[var(--app-bg)] text-slate-900 dark:text-[#F1FAEE]'}`}>
      <header className={`p-6 sticky top-0 z-20 shadow-xl transition-colors duration-500 flex items-center justify-between ${isGhostMode ? 'bg-[var(--app-bg-ghost)]' : 'bg-[var(--app-primary)] text-white'}`}>
        ...
        <button onClick={handleSave} className={`p-2 rounded-xl transition-colors ${isGhostMode ? 'text-[var(--app-accent-light)] hover:bg-white/10' : 'text-[var(--app-accent)] hover:bg-white/10'}`}>
          <Save size={24} />
        </button>
      </header>
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-8">
        ...
        <div className="relative h-40 rounded-[2rem] overflow-hidden group" onClick={handleBannerClick}>
          ...
          <div className={`absolute inset-0 bg-black/40 flex items-center justify-center transition-opacity cursor-pointer ${isUploadingBanner ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}`}>
            <Camera size={32} className={`text-slate-900 dark:text-[#F1FAEE] ${isUploadingBanner ? 'animate-bounce' : ''}`} />
          </div>
        </div>
        ...
        <div className="flex justify-center -mt-16 relative z-10">
          <div className="relative group" onClick={handleAvatarClick}>
            <div className={`w-32 h-32 rounded-[2.5rem] p-1 transition-all duration-500 ${isGhostMode ? 'bg-[var(--app-bg-ghost)]' : 'bg-[var(--app-primary)]'} ${isUploadingAvatar ? 'scale-95' : ''}`}>
            ...
            <div className={`absolute inset-0 flex items-center justify-center transition-opacity cursor-pointer ${isUploadingAvatar ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}`}>
              <div className="bg-black/50 p-3 rounded-full backdrop-blur-sm">
                <Camera size={24} className={`text-slate-900 dark:text-[#F1FAEE] ${isUploadingAvatar ? 'animate-spin' : ''}`} />
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-6">
          <div className="space-y-2">
            <label className={`text-[10px] font-black uppercase tracking-widest ml-4 ${isGhostMode ? 'text-[var(--app-accent-light)]' : 'text-slate-500 dark:text-slate-400'}`}>Display Name</label>
            <div className={`flex items-center gap-3 p-4 rounded-2xl border ${isGhostMode ? 'bg-[var(--app-bg-ghost)] border-[color-mix(in_srgb,var(--app-accent)_20%,transparent)]' : 'bg-[var(--app-bg-surface)] border-white/10'}`}>
              ...
              <input 
                ...
                className="flex-1 bg-transparent focus:outline-none text-sm font-bold text-slate-900 dark:text-[#F1FAEE]"
              />
            </div>
          </div>

          <div className="space-y-2">
            <label className={`text-[10px] font-black uppercase tracking-widest ml-4 ${isGhostMode ? 'text-[var(--app-accent-light)]' : 'text-slate-500 dark:text-slate-400'}`}>Username</label>
            <div className={`flex items-center gap-3 p-4 rounded-2xl border ${isGhostMode ? 'bg-[var(--app-bg-ghost)] border-[color-mix(in_srgb,var(--app-accent)_20%,transparent)]' : 'bg-[var(--app-bg-surface)] border-white/10'}`}>
          ...
          <div className="space-y-2">
            <label className={`text-[10px] font-black uppercase tracking-widest ml-4 ${isGhostMode ? 'text-[var(--app-accent-light)]' : 'text-slate-500 dark:text-slate-400'}`}>Bio</label>
            <div className={`flex items-start gap-3 p-4 rounded-2xl border ${isGhostMode ? 'bg-[var(--app-bg-ghost)] border-[color-mix(in_srgb,var(--app-accent)_20%,transparent)]' : 'bg-[var(--app-bg-surface)] border-white/10'}`}>
              ...
              <textarea 
                ...
                className="flex-1 bg-transparent focus:outline-none text-sm font-medium resize-none h-24 text-slate-900 dark:text-[#F1FAEE]"
                maxLength={150}
              />
            </div>
            <p className="text-[9px] text-right opacity-40 font-mono text-slate-900 dark:text-[#F1FAEE]">{bio.length}/150</p>
          </div>
        </div>
        ...
        <div className="pt-8 border-t border-white/10">
          <div className="flex items-center gap-2 mb-6">
            ...
            <h3 className="text-lg font-bold font-montserrat text-slate-900 dark:text-[#F1FAEE]">Vista Profile Card</h3>
          </div>
          ...
          <div className="space-y-3">
            <label className={`text-[10px] font-black uppercase tracking-widest ml-4 flex items-center gap-2 ${isGhostMode ? 'text-[var(--app-accent-light)]' : 'text-slate-500 dark:text-slate-400'}`}>
