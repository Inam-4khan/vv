    ...
    >
      ...
      >
        <div className={`flex items-center gap-2.5 px-4 py-2 rounded-full border backdrop-blur-md shadow-md text-xs font-bold ${
          isGhostMode 
            ? 'bg-[#062B34]/90 border-purple-500/30 text-purple-300' 
      ...
      <header className={`p-6 sticky top-0 z-50 shadow-md transition-colors duration-500 text-white ${isGhostMode ? 'bg-[#062B34]' : 'bg-primary'}`}>
        <div className="flex justify-between items-center animate-fade-in">
          <div className="flex items-center gap-3">
            <div className="animate-vizu-logo-entrance inline-flex items-center justify-center">
              <BrandLogo size={32} color={isGhostMode ? '#2EC4B6' : '#FFF9E6'} />
            </div>
      ...
      <div className="px-4 py-6 overflow-x-auto no-scrollbar flex gap-4" role="region" aria-label="Stories feed">
        ...
        >
          <div className="relative">
            <div className={`w-16 h-16 rounded-full p-0.5 bg-gradient-to-tr ${isGhostMode ? 'from-purple-600 to-indigo-500' : 'from-secondary to-[#2EC4B6]'} group-hover:scale-110 transition-transform duration-300`}>
              <div className={`w-full h-full rounded-full border-2 ${isGhostMode ? 'border-[#0A001F] bg-[#150036]' : 'border-[var(--app-bg,#FFF9E6)] bg-white'} overflow-hidden flex items-center justify-center`}>
            ...
            <div className={`absolute bottom-0 right-0 p-1 rounded-full border-2 text-white ${isGhostMode ? 'bg-purple-600 border-[#0A001F]' : 'bg-secondary border-[var(--app-bg,#FFF9E6)]'}`}>
              <Plus size={10} />
            </div>
          </div>
        ...
        {stories.map((story, index) => (
          ...
          >
            <div className={`w-16 h-16 rounded-full p-0.5 transition-all duration-300 group-hover:scale-110 ${story.isSeen ? 'bg-gray-250 opacity-60' : `ring-2 ring-offset-2 ${isGhostMode ? 'bg-gradient-to-tr from-purple-600 to-indigo-500 ring-purple-500/20' : 'bg-gradient-to-tr from-secondary to-[#2EC4B6] ring-secondary/20'}`}`}>
              <div className={`w-full h-full rounded-full border-2 ${isGhostMode ? 'border-[#0A001F] bg-[#150036]' : 'border-[var(--app-bg,#FFF9E6)] bg-white'} overflow-hidden`}>
      ...
      <div className="px-4 pb-6 space-y-6">
        {MOCK_POSTS.map((post, index) => (
          <div 
            key={post.id} 
            className={`rounded-[2.5rem] overflow-hidden shadow-xl border animate-fade-in transition-all duration-550 ${isGhostMode ? 'bg-[#150036] border-purple-500/10' : 'bg-white border-black/5'}`}
            style={{ animationDelay: `${(index + 1) * 110}ms`, animationFillMode: 'both' }}
          >
            <div className="p-5 flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className={`w-12 h-12 rounded-2xl border-2 bg-gray-200 overflow-hidden ${isGhostMode ? 'border-purple-500/40' : 'border-secondary'}`}>
            ...
            <div className="p-6">
              ...
              <div className={`flex items-center justify-between pt-4 border-t ${isGhostMode ? 'border-white/5' : 'border-primary/5'}`}>
                <div className="flex items-center gap-4">
                  ...
                  <button className={`flex items-center gap-1.5 transition-colors ${isGhostMode ? 'text-white/30 hover:text-purple-400' : 'text-primary/30 hover:text-secondary'}`} title="Share to Hush">
                    <Send size={18} />
                ...
                <div className="flex items-center gap-2">
                  <button 
                    ...
                    className={`p-1.5 transition-colors ${isGhostMode ? 'text-white/20 hover:text-purple-400' : 'text-primary/20 hover:text-secondary'}`} 
                    title="Post Options"
                  >
