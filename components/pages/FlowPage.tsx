...
import { MoreVertical, Compass, Plus, Music, Share2, Heart, MessageCircle, Send, MoreHorizontal, Bookmark, Bell, RefreshCw } from 'lucide-react';
  ...
  return (
    <div 
      ...
      className={`min-h-full transition-all duration-700 pb-24 ${
        isGhostMode ? 'bg-[#020F14] text-[#F1FAEE]' : 'bg-[var(--app-bg)] text-white'
      }`}
    >
      ...
      >
        <div className={`flex items-center gap-2.5 px-4 py-2 rounded-full border backdrop-blur-md shadow-md text-xs font-bold ${
          isGhostMode 
            ...
            : 'bg-white/90 border-secondary/20 text-[#062B34]'
        }`}>
          <RefreshCw 
            size={16} 
      ...
      <header className={`p-6 sticky top-0 z-50 shadow-md transition-colors duration-500 text-white ${isGhostMode ? 'bg-[#020F14]' : 'bg-[#062B34]'}`}>
        ...
        >
          <div className="relative">
            <div className={`w-16 h-16 rounded-full p-0.5 bg-gradient-to-tr ${isGhostMode ? 'from-[#2EC4B6] via-[#80FFEC] to-[#2EC4B6]' : 'from-secondary to-[#2EC4B6]'} group-hover:scale-110 transition-transform duration-300`}>
              <div className={`w-full h-full rounded-full border-2 ${isGhostMode ? 'border-[#041018] bg-[#031820]' : 'border-[#062B34] bg-[#0A2832]'} overflow-hidden flex items-center justify-center`}>
            ...
            <div className={`absolute bottom-0 right-0 p-1 rounded-full border-2 text-white ${isGhostMode ? 'bg-[#2EC4B6] border-[#041018]' : 'bg-secondary border-[#062B34]'}`}>
              <Plus size={10} />
            </div>
          </div>
          <span className={`text-[10px] font-bold uppercase ${isGhostMode ? 'text-white/40' : 'text-white/40'}`}>My Story</span>
        </button>
        {stories.map((story, index) => (
          ...
          >
            <div className={`w-16 h-16 rounded-full p-0.5 transition-all duration-300 group-hover:scale-110 ${story.isSeen ? 'bg-gray-250 opacity-60' : `ring-2 ring-offset-2 ${isGhostMode ? 'bg-gradient-to-tr from-[#2EC4B6] via-[#80FFEC] to-[#2EC4B6] ring-[#2EC4B6]/20' : 'bg-gradient-to-tr from-secondary to-[#2EC4B6] ring-secondary/20'}`}`}>
              <div className={`w-full h-full rounded-full border-2 ${isGhostMode ? 'border-[#041018] bg-[#031820]' : 'border-[#062B34] bg-[#0A2832]'} overflow-hidden`}>
                <OptimizedImg src={story.avatar} alt={`@${story.username}`} width={64} height={64} loading="lazy" className="w-full h-full object-cover" />
              </div>
            </div>
            <span className={`text-[10px] font-bold truncate w-16 text-center ${isGhostMode ? 'text-white/80' : 'text-white'}`}>@{story.username}</span>
          </button>
        ))}
      </div>
      ...
      <div className="px-4 pb-6 space-y-6">
        {MOCK_POSTS.map((post, index) => (
          <div 
            key={post.id} 
            className={`rounded-[2.5rem] overflow-hidden shadow-xl border animate-fade-in transition-all duration-550 ${isGhostMode ? 'bg-[#031820] border-[#2EC4B6]/10' : 'bg-[#0A2832] border-white/10'}`}
            style={{ animationDelay: `${(index + 1) * 110}ms`, animationFillMode: 'both' }}
          >
            <div className="p-5 flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className={`w-12 h-12 rounded-2xl border-2 bg-gray-200 overflow-hidden ${isGhostMode ? 'border-[#2EC4B6]/30' : 'border-secondary'}`}>
                ...
                <div>
                  ...
                  <p className="text-[10px] font-bold uppercase tracking-widest text-white/40">{post.timestamp}</p>
                </div>
              </div>
              <button className={`p-2 ${isGhostMode ? 'text-white/20 hover:text-white' : 'text-white/20 hover:text-white'}`}><MoreVertical size={20} /></button>
            </div>
            ...
            <div className="p-6">
              <p className={`mb-6 text-sm leading-relaxed ${isGhostMode ? 'text-white/70' : 'text-white/70'}`}>{post.content}</p>
              
              <div className="flex items-center justify-between pt-4 border-t border-white/10">
                <div className="flex items-center gap-4">
                  <button onClick={() => toggleLike(post.id)} className={`flex items-center gap-1.5 transition-all active:scale-90 ${likedPosts.includes(post.id) ? 'text-red-500' : isGhostMode ? 'text-white/40 hover:text-white' : 'text-white/40 hover:text-white'}`}>
                  ...
                  <button className="flex items-center gap-1.5 text-white/40">
                    <MessageCircle size={20} />
                    <span className="text-[11px] font-black">{post.comments}</span>
                  </button>
                  <button className={`flex items-center gap-1.5 transition-colors ${isGhostMode ? 'text-white/30 hover:text-[#80FFEC]' : 'text-white/30 hover:text-secondary'}`} title="Share to Hush">
                    <Send size={18} />
                ...
                <div className="flex items-center gap-2">
                  <button 
                    onClick={() => {
                      const action = confirm(`Post Options:\n1. Save Post\n2. Share Externally\n3. Report`);
                    }} 
                    className={`p-1.5 transition-colors ${isGhostMode ? 'text-white/20 hover:text-[#80FFEC]' : 'text-white/20 hover:text-secondary'}`} 
                    title="Post Options"
                  >
