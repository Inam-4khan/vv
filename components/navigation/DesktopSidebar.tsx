    ...
    >
              ...
              >
                <Icon
                  ...
                  className={`transition-transform duration-200 group-hover:scale-110 ${
                    isActive ? (isGhostActive ? 'text-[#062B34]' : 'text-[var(--app-accent)]') : ''
                  }`}
                />
      ...
      <div className="pt-4 border-t border-white/10 space-y-2">
        {onToggleGhost && (
          ...
          >
            <div className="flex items-center gap-2.5">
              <Ghost size={18} aria-hidden="true" className={isGhostActive ? 'text-[var(--app-accent-light)] animate-pulse' : 'text-white/80'} />
              <span>Ghost Mode</span>
            </div>
            <span className={`px-2 py-0.5 rounded-full text-[9px] uppercase font-mono ${
              isGhostActive ? 'bg-[color-mix(in_srgb,var(--app-accent)_30%,transparent)] text-[var(--app-accent-light)]' : 'bg-white/10 text-white/80'
            }`}>
        ...
        {onToggleTheme && (
          ...
          >
            <div className="flex items-center gap-2.5">
              {isDarkMode ? <Sun size={18} className="text-amber-300" aria-hidden="true" /> : <Moon size={18} className="text-white/80" aria-hidden="true" />}
              <span>Theme Mode</span>
            </div>
            <span className={`px-2 py-0.5 rounded-full text-[9px] uppercase font-mono ${
              isDarkMode ? 'bg-amber-500/30 text-amber-300' : 'bg-white/10 text-white/80'
            }`}>
