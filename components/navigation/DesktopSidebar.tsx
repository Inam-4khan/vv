      ...
      <div className="space-y-8">
        ...
        >
          <div className={`p-2 rounded-2xl transition-all duration-300 ${
            isGhostActive ? 'bg-[#2EC4B6]/20 text-[#80FFEC]' : 'bg-white/10 text-[#FFF9E6]'
          }`}>
          ...
          {navItems.map((item) => {
            ...
            return (
              <button
                ...
                className={`w-full flex items-center gap-4 px-4 py-3 rounded-2xl text-sm font-bold tracking-tight transition-all duration-200 group focus:outline-none focus-visible:ring-2 focus-visible:ring-[#2EC4B6] ${
                  isActive
                    ? isGhostActive
                      ? 'bg-[#2EC4B6] text-[#062B34] font-black shadow-md'
                      : 'bg-white/15 text-[#FFF9E6] border border-white/10 shadow-md'
                    : isGhostActive
              ...
              >
                <Icon
                  ...
                  className={`transition-transform duration-200 group-hover:scale-110 ${
                    isActive ? (isGhostActive ? 'text-[#062B34]' : 'text-[#2EC4B6]') : ''
                  }`}
                />
      ...
      <div className="pt-4 border-t border-white/10 space-y-2">
        {onToggleGhost && (
          <button
            ...
            className={`w-full flex items-center justify-between px-4 py-3 rounded-2xl text-xs font-bold transition-all border focus:outline-none focus-visible:ring-2 focus-visible:ring-[#80FFEC] focus-visible:ring-offset-2 focus-visible:ring-offset-[#062B34] ${
              isGhostActive
                ? 'bg-[#062B34] text-[#80FFEC] border-[#2EC4B6]/40 shadow-inner'
                : 'bg-white/5 text-white/70 border-white/10 hover:bg-white/10'
            }`}
          >
            <div className="flex items-center gap-2.5">
              <Ghost size={18} aria-hidden="true" className={isGhostActive ? 'text-[#80FFEC] animate-pulse' : 'text-white/50'} />
              <span>Ghost Mode</span>
            </div>
            <span className={`px-2 py-0.5 rounded-full text-[9px] uppercase font-mono ${
              isGhostActive ? 'bg-[#2EC4B6]/30 text-[#80FFEC]' : 'bg-white/10 text-white/40'
            }`}>
